...
export function Header({
  ...
  const submitSearch = (event: FormEvent<HTMLFormElement>) => {
    ...
    router.push(trimmed ? `/ko/templates/OHMT032-community/board?q=${encodeURIComponent(trimmed)}` : '/ko/templates/OHMT032-community/board')
  }
        ...
        <div className="mx-auto flex h-full max-w-[1440px] items-center justify-between gap-3 px-4 lg:px-6">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              <p className="truncate text-sm font-semibold text-white">커뮤니티 운영을 더 빠르게 시작하세요</p>
              <p className="mt-0.5 hidden truncate text-xs text-white/64 sm:block">공지, 질문, 후기 구조가 준비된 커뮤니티 템플릿</p>
            </div>
          </div>
          <button
            type="button"
