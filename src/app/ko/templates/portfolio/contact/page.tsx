    ...
    <div className="border-b border-[var(--color-border)]">
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            <p className="text-[0.88rem] text-[var(--color-text-muted)] leading-relaxed pb-6 pr-8">{a}</p>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
        ...
        <section className="pt-40 pb-20 border-b border-[var(--color-border)]">
          <div className="max-w-[1440px] mx-auto px-8 md:px-10">
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-10">
              <div>
                <span className="text-[13px] font-medium tracking-[0.3em] uppercase text-[var(--color-text-muted)] block mb-6">문의?�기</span>
            ...
            <div className="max-w-[680px]">
              {submitted ? (
                <motion.div
                  ...
                  className="py-16"
                >
                  <h2 className="text-[2rem] font-black uppercase tracking-tighter mb-3">메시지 ?�송 ?�료</h2>
                  <p className="text-[var(--color-text-muted)] text-[0.9rem]">?�업??기�? 1???�내???��??�리겠습?�다.</p>
                </motion.div>
              ) : (
                <form onSubmit={handleSubmit} className="space-y-0">
                  {[
                    { label: '?�름', type: 'text', placeholder: '?�함???�력?�주?�요' },
                    { label: '?�메??, type: 'email', placeholder: 'you@company.com' },
                  ].map(field => (
          ...
          <div className="max-w-[1440px] mx-auto px-8 md:px-10">
            <div className="grid md:grid-cols-2 gap-16">
              <div>
                ...
                <h2 className="text-[clamp(1.8rem,4vw,3rem)] font-black tracking-tighter leading-tight">?�주 묻는 질문</h2>
              </div>
