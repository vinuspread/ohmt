...
export default function PopularServices() {
  ...
  return (
    <section className="relative overflow-hidden bg-[var(--color-bg-secondary)] py-24 lg:py-32">
      ...
      <div className="overflow-hidden">
        <div className="animate-marquee-right flex gap-6 w-max" style={{ animationDuration: "25s" }}>
          {duplicated.map((service, i) => (
            <a key={`${service.id}-${i}`} href={`/ko/templates/OHMT026-spa-KO/service#${service.id}`} className="group w-[280px] shrink-0 rounded-2xl bg-[var(--color-bg)] border border-[var(--color-border)] overflow-hidden hover:-translate-y-1 transition-transform duration-300">
      ...
      <style>{`@keyframes marquee-right { from { transform: translateX(-50%); } to { transform: translateX(0); } } .animate-marquee-right { animation: marquee-right linear infinite; } @media (prefers-reduced-motion: reduce) { .animate-marquee-right { animation: none; } }`}</style>
    </section>
  );
}
