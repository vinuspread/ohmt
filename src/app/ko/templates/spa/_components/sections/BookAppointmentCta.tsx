      ...
      <div className="mx-auto max-w-7xl px-6 text-center">
        <motion.div initial={reduce ? { opacity: 0 } : { opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ duration: 0.6, ease: EASE_OUT }}>
          ...
          <div className="mt-10 flex flex-col sm:flex-row gap-4 justify-center">
            <a href="/ko/templates/OHMT026-spa-KO/contact" className="inline-flex items-center rounded-full bg-[var(--color-primary)] text-[var(--color-text-contrast)] px-8 py-3.5 text-sm font-semibold hover:brightness-110 active:scale-[0.97] transition-all duration-150">예약하기</a>
            <a href="/ko/templates/OHMT026-spa-KO/pricing" className="inline-flex items-center rounded-full border border-white/20 text-[var(--color-text-contrast)] px-8 py-3.5 text-sm font-semibold hover:bg-white/10 transition-all duration-150">가격 보기</a>
          </div>
        </motion.div>
      </div>
    </section>
