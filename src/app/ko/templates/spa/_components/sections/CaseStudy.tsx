      ...
      <div className="mx-auto max-w-4xl px-6">
        ...
        <div className="space-y-3">
          {caseStudies.map((cs, i) => {
            ...
            return (
              <motion.div key={cs.id} initial={reduce ? { opacity: 0 } : { opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-60px" }} transition={{ duration: 0.4, delay: i * 0.1, ease: EASE_OUT }} className="rounded-2xl bg-[var(--color-bg)] border border-[var(--color-border)] overflow-hidden">
                <button onClick={() => setOpenIndex(isOpen ? null : i)} className="w-full flex items-center justify-between p-6 lg:p-8 text-left">
                  ...
                  <span className="text-lg transition-transform duration-300 shrink-0 ml-4" style={{ transform: isOpen ? "rotate(45deg)" : "rotate(0deg)" }}>+</span>
                </button>
