    ...
    <section className="py-12 md:py-24 lg:py-36 bg-white selection:bg-black selection:text-white">
      <div className="max-w-[1440px] mx-auto px-6 md:px-12">
        <div className="flex justify-between items-end mb-16 md:mb-24 gap-4">
          ...
          <Link href="/ko/templates/OHMT001-fashion/category/collection" className="group flex items-center gap-2 text-[11px] sm:text-[12px] font-medium uppercase tracking-[0.15em] sm:tracking-[0.2em] border-b border-black/10 pb-1 hover:border-black transition-colors whitespace-nowrap">
            전체 보기
          </Link>
        </div>
            ...
            return (
              ...
              >
                <div className="aspect-square bg-[var(--color-bg-secondary)] overflow-hidden mb-4 sm:mb-8 relative">
                  <Link href={`/ko/templates/OHMT001-fashion/product/${p.id}`} className="block w-full h-full">
                    <img 
                      src={p.image} 
                      alt={p.name}
                ...
                <div className="space-y-1 transition-transform duration-500 ease-out group-hover:-translate-y-1">
                  <Link href={`/ko/templates/OHMT001-fashion/product/${p.id}`}>
                    <h3 className="text-[15px] font-bold uppercase tracking-normal line-clamp-1">{p.name}</h3>
                  </Link>
