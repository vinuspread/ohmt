    ...
    <>
      <nav className={`fixed top-0 left-0 right-0 z-50 h-14 md:h-20 transition-[background,border-color] duration-700 flex items-center ${!isTransparent ? "bg-white/95 backdrop-blur-xl border-b border-black/5" : "bg-transparent"}`}>
        <div className="max-w-[1440px] mx-auto w-full px-6 md:px-12 flex justify-between items-center">
          <Link href="/ko/templates/OHMT001-fashion-KO" className={`text-xl md:text-2xl font-black tracking-tighter uppercase transition-colors duration-500 ${!isTransparent ? "text-black" : "text-white"}`}>
            Oh My Template
          </Link>
           ...
           <div className={`hidden md:flex items-center gap-10 text-[14px] font-bold uppercase tracking-[0.3em] transition-colors duration-500 ${!isTransparent ? "text-black/60" : "text-white/80"}`}>
             {[
                { name: "컬렉션", id: "collection", href: "/ko/templates/OHMT001-fashion-KO/category/collection" },
                { name: "아카이브", id: "archive", href: "/ko/templates/OHMT001-fashion-KO/category/archive" },
                { name: "저널", id: "journal", href: "/ko/templates/OHMT001-fashion-KO/category/journal" },
                { name: "브랜드", id: "about", href: "/ko/templates/OHMT001-fashion-KO/category/about" },
              ].map((item) => {
                const isActive = pathname.includes(`/ko/templates/OHMT001-fashion-KO/category/${item.id}`);
               return (
                 <Link
                   key={item.id}
                   href={item.href}
           ...
           <div className={`flex items-center gap-4 sm:gap-6 transition-colors duration-500 ${!isTransparent ? "text-black" : "text-white"}`}>
             ...
             <Link href="/ko/templates/OHMT001-fashion-KO/cart" className="relative hover:opacity-50 transition-opacity">
          ...
          {/* Utilities */}
           <div className="flex flex-col gap-3">
             <span className="text-[12px] font-bold tracking-[0.2em] text-black/30 mb-3 block">서비스</span>
              <Link
                href="/ko/templates/OHMT001-fashion-KO/cart"
                onClick={() => setMobileOpen(false)}
                className="hover:text-black py-2.5 transition-colors block"
              >
                쇼핑백
              ...
              <Link
                href="/ko/templates/OHMT001-fashion-KO"
                onClick={() => setMobileOpen(false)}
