...
const products = [
  {
    image: 'product-1.jpg',
    alt: '어두운 스톤 카운터 위 브러시드 스틸 바디와 노출 그룹 헤드를 가진 NOVA 에스프레소 머신',
    label: '1.5L 필터 물탱크',
    desc: '카본 필터가 포함된 1.5L 탈착식 물탱크입니다.',
  },
  {
    image: 'product-2.jpg',
    alt: '작은 컵으로 흐르는 크레마와 에스프레소 추출 클로즈업',
    label: '±0.5°C PID 제어',
    desc: '연속 추출 시에도 설정 온도 범위를 유지하도록 제어합니다.',
  },
  {
    image: 'product-3.jpg',
    alt: '어두운 스톤 주방 카운터에서 NOVA 그룹 헤드에 포터필터를 장착하는 손',
    label: '9bar 로터리 펌프',
    desc: '일반적인 진동 펌프보다 소음과 진동을 줄이고 9bar 추출 압력을 제공합니다.',
  },
]
      ...
      <div className="mx-auto max-w-[1440px]">
        ...
        >
          ...
          <p className="max-w-[560px] text-sm leading-relaxed text-[var(--color-text-muted)] md:col-span-7 md:justify-self-end md:text-sm">
            본체 크기, 추출 온도, 압력 게이지처럼 실제 사용에 영향을 주는 요소를 먼저 보여줍니다.</p>
        </motion.div>
          ...
          {products.map((p, i) => (
            ...
            >
              ...
              <div className="mt-4 flex items-start justify-between gap-4 border-t border-[var(--color-border)] pt-4">
                ...
                <Link
                  href={`${base}/product/nova-dual-boiler`}
