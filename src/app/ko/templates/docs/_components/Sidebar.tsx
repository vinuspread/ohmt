...
function PageTreeItem({ node, depth, onNavigate }: { node: TreeNode; depth: number; onNavigate: () => void }) {
  ...
  const isActive = pathname === `/ko/templates/OHMT028-docs-kr/${slug}` || pathname === `/ko/templates/OHMT028-docs-kr`;
  ...
  return (
    <div className="relative">
      ...
      <Link
        href={`/ko/templates/OHMT028-docs-kr/${slug}`}
        onClick={onNavigate}
        className={`flex items-center gap-1.5 py-1.5 rounded-md transition-all duration-150 relative ${
          ...
          isActive
            ...
            : isRoot
              ? "text-[var(--color-text-secondary)] font-medium hover:text-[var(--color-text)] hover:underline"
  ...
  return (
    <aside className="w-[260px] flex-shrink-0 h-screen border-r border-[var(--color-border)] flex flex-col overflow-hidden bg-[var(--color-bg)]">
      <div className="px-4 h-14 flex items-center border-b border-[var(--color-border)]">
        <Link href="/ko/templates/OHMT028-docs-kr" onClick={handleNav} className="flex items-center gap-2 px-2 text-sm font-bold text-[var(--color-text)] rounded-md transition-colors duration-150">
      ...
      <nav className="flex-1 overflow-y-auto px-3 pb-4 pt-2">
        {filteredTree.map((node, index) => (
          <div key={node.page.slug}>
            {index > 0 && <div className="h-px bg-[var(--color-border-light)] mx-2 my-2" />}
        ...
        {filteredTree.length === 0 && !searchQuery && (
          <div className="px-2 py-8 text-center">
            <p className="text-sm text-[var(--color-text-muted)]">페이지를 찾을 수 없습니다</p>
          </div>
        )}
      </nav>
