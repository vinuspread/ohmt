// src/app/templates/car/-components/sections/FeatureGrid.tsx
"use client";
...
export const SplitSection = ({ eyebrow, title, desc, img, reverse }: SplitProps) => (
  <div className={clsx("grid md:grid-cols-2 min-h-[620px] bg-black overflow-hidden", reverse && "md:[direction:rtl]")}>
    <div className={clsx(
      ...
      <h2 className="text-[clamp(2rem,3.5vw,3.2rem)] font-extrabold tracking-[-0.03em] leading-[1.2] mb-5 text-white whitespace-pre-line break-keep">
        {title}
      </h2>
      <p className="text-[0.88rem] text-[var(--theme-text-muted)] leading-[1.8] font-normal mb-10 max-w-[380px]">
        {desc}
      </p>
      <button type="button" className="inline-flex items-center gap-3 text-[0.72rem] font-semibold uppercase tracking-[0.16em] text-white hover:translate-x-1.5 transition-all duration-500 text-left">
        자세히 알아보기 <span className="text-[1.2em]">→</span>
      </button>
    </div>
...
export const FullBleedSection = ({ eyebrow, title, desc, img }: Omit<SplitProps, "reverse">) => (
  <div className="relative h-[75vh] min-h-[500px] overflow-hidden">
    ...
    <div className="absolute bottom-0 left-0 right-0 px-6 py-8 md:px-[var(--theme-gutter)] md:pb-20 max-w-[var(--theme-container)] mx-auto">
      ...
      <h2 className="text-[clamp(2rem,4vw,4rem)] font-bold tracking-[-0.03em] leading-[1.2] mb-4 max-w-[600px] text-white whitespace-pre-line break-keep">
        {title}
      </h2>
      <p className="text-[0.88rem] text-[var(--theme-text-muted)] font-normal mb-8 max-w-[400px]">
        {desc}
      </p>
      <button type="button" className="inline-flex items-center gap-3 text-[0.72rem] font-semibold uppercase tracking-[0.16em] text-white hover:translate-x-1.5 transition-all duration-500">
        사양 살펴보기 <span className="text-[1.2em]">→</span>
      </button>
    </div>
  </div>
);
