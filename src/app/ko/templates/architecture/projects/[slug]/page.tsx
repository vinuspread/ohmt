// src/app/en/templates/OHMT027-architecture-KO/projects/[slug]/page.tsx
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      title: `${project.title} - Architecture Portfolio - Oh My Template`,
      description: project.description,
      url: `https://ohmytemplate.com/en/templates/OHMT027-architecture-KO/projects/${slug}`,
      images: [{ url: project.image }],
    },
    alternates: {
      canonical: `https://ohmytemplate.com/en/templates/OHMT027-architecture-KO/projects/${slug}`,
      languages: {
      ...
      <div className="relative min-h-screen bg-white text-[#1A1A1A] font-sans antialiased overflow-hidden selection:bg-[#1A1A1A] selection:text-white">
        ...
        <main className="pt-32 pb-24">
          ...
          <div className="max-w-[1440px] mx-auto px-8 md:px-12 lg:px-20 py-8">
            <Link
              href="/en/templates/OHMT027-architecture-KO/projects"
              className="font-sans text-[12px] font-medium tracking-[0.1em] text-[#888888] hover:text-[#1A1A1A] transition-colors"
            >
              ← BACK TO PORTFOLIO
            </Link>
          </div>
          ...
          <section className="max-w-[1440px] mx-auto px-8 md:px-12 lg:px-20 pb-12">
            <ScrollReveal>
              <div className="space-y-4">
                ...
                <h1 className="font-heading font-normal text-[48px] md:text-[60px] lg:text-[72px] leading-tight text-[#1A1A1A]">
                  {project.title}
                </h1>
              </div>
            </ScrollReveal>
                ...
                <div className="border-t border-[#E0E0E0] divide-y divide-[#E0E0E0]">
                  {specs.map((spec, i) => (
                    <div key={i} className="py-4 flex justify-between items-baseline text-[14px]">
                      <span className="font-sans font-semibold text-[#888888] tracking-wider uppercase">
                        {spec.label}
                      </span>
                      <span className="font-sans text-[#1A1A1A] font-medium text-right pl-4">
                        {spec.value}
                      </span>
                    </div>
                  ))}
            ...
            <ScrollReveal>
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                <div className="lg:col-span-8 space-y-6">
                  <h2 className="font-heading font-normal text-[28px] md:text-[32px] leading-[1.5] text-[#1A1A1A]">
                    {project.caption}
                  </h2>
                  <p className="font-sans text-[16px] leading-[1.8] text-[#888888]">
                    {project.description}
                  </p>
                </div>
              </div>
