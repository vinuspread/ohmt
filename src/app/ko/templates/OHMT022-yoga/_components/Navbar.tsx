        ...
        <ul className="hidden md:flex items-center gap-10">
          {NAV_ITEMS.slice(1).map((item) => (
            <li key={item.href}>
              <Link
                href={item.href}
                className={`text-xs tracking-tight transition-colors duration-300 font-normal ${
                  isVisible
            ...
            {NAV_ITEMS.map((item) => (
              <li key={item.href} className="pt-5 first:pt-0">
                <Link
                  ...
                  className="text-xs tracking-[0.1em] text-[var(--color-text-muted)] hover:text-[var(--color-text)] transition-colors font-normal"
                  style={{ fontFamily: "var(--font-body)" }}
                >
                  {item.label}
                </Link>
