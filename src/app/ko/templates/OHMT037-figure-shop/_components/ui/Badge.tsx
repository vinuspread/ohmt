...
import { statusLabel, type FigureStatus } from '../../data/figures'
...
export function Badge({ variant = 'neutral', children }: { variant?: BadgeVariant; children: React.ReactNode }) {
  return (
    <span
      className={clsx(
        'inline-flex min-h-7 items-center justify-center rounded-full border px-3 text-[11px] font-semibold leading-none',
        styles[variant]
      )}
    >
      {children}
