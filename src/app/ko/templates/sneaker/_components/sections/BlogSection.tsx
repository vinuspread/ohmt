    ...
    <section className="py-16 bg-white">
      <div className="max-w-[1440px] mx-auto px-6">
        <div className="flex items-center justify-between mb-10">
          ...
          <Link href={`/ko/templates/sneaker/blog`} className="text-[0.78rem] font-bold uppercase tracking-[0.1em] text-black/50 hover:text-black transition-colors border-b border-black/20 pb-0.5">
            전체 포스트
          </Link>
        </div>
        <div className="grid md:grid-cols-4 gap-6">
          {posts.map((p, i) => (
            <Link key={slugs[i]} href={`/ko/templates/sneaker/blog`} className="group block">
