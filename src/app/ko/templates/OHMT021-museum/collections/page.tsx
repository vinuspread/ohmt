                ...
                >
                  <Link href={`/ko/templates/OHMT021-museum/collections/${item.slug}`} className="block w-full h-full cursor-pointer">
                    ...
                    <div className="absolute top-8 left-8 mix-blend-difference overflow-hidden">
                       <span className="text-xs opacity-50 block group-hover:-translate-y-full transition-transform duration-500">{"소장품"}</span>
                    </div>
                  </Link>
                </motion.div>
            ))}
...
export default function CollectionsPage(props: Record<string, unknown>) {
  return (
