...
function ContentBlock({ block }: { block: DocPage["content"][number] }) {
  switch (block.type) {
    case "heading": {
      ...
      return (
        <Tag
          className={`${sizeClass} font-bold text-[var(--color-text)]`}
    ...
    case "text":
      return (
        <p className="mb-4 text-base leading-[var(--leading-body)] text-[var(--color-text-secondary)]">
          {block.value}
        </p>
      );
    case "list":
      return (
        <ul className="mb-4 space-y-2">
          {block.items?.map((item) => (
            <li
              key={item}
              className="flex gap-2 text-base leading-relaxed text-[var(--color-text-secondary)]"
            >
    ...
    case "quote":
      return (
        <blockquote className="mb-4 border-l-2 border-[var(--color-quote-border)] pl-4 text-base italic text-[var(--color-text-secondary)]">
          {block.value}
        </blockquote>
      );
    case "callout":
      return (
        <div className="mb-4 rounded-lg border border-[var(--color-callout-border)] bg-[var(--color-callout-bg)] px-4 py-3 text-sm leading-relaxed text-[var(--color-text-secondary)]">
          {block.value}
        </div>
      );
  }
...
export function DocContent({
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8 md:px-8 md:py-16">
      ...
      <h1
        className="mb-3 text-3xl font-bold leading-tight text-[var(--color-text)] md:text-4xl"
        style={{ fontFamily: "var(--font-heading)" }}
      >
        {page.title}
      </h1>
      <p className="mb-12 max-w-2xl text-base leading-relaxed text-[var(--color-text-muted)]">
        {page.description}
      </p>
