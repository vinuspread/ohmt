    ...
    <header className="bg-white">
      ...
      <div className="max-w-[1280px] mx-auto px-4 md:px-8 py-5 md:py-6">
        <div className="flex flex-col items-center gap-3 md:flex-row md:justify-between md:items-start md:mb-3">
          ...
          <Link href="/ko/templates/OHMT013-newspaper" className="font-[family-name:var(--theme-font-heading)] text-[clamp(2.2rem,13vw,4.5rem)] font-black tracking-tighter leading-none text-center md:flex-1">
             OHMT TIMES
          </Link>
          ...
          <div className="flex h-11 overflow-x-auto">
            {navItems.map((item) => (
              <Link
                key={item.name}
                href={item.href}
                className="shrink-0 px-4 h-full flex items-center text-[0.78rem] font-bold uppercase tracking-wider text-black border-r border-[var(--color-border)] hover:bg-black hover:text-white transition-colors"
              >
                {item.name}
              </Link>
            ))}
