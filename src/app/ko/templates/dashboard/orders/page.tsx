    ...
    >
      <div className="p-6">
        ...
        <div className="space-y-4">
          <div>
            <p className="text-xs text-[var(--color-text-muted)] mb-1">날짜</p>
            <p className="text-sm text-[var(--color-text)]">{order.date}, 2026</p>
          </div>
          <div>
            <p className="text-xs text-[var(--color-text-muted)] mb-1">고객</p>
            <p className="text-sm text-[var(--color-text)]">{order.customer}</p>
          </div>
          <div>
            <p className="text-xs text-[var(--color-text-muted)] mb-1">이메일</p>
            <p className="text-sm text-[var(--color-text)]">{order.email}</p>
          </div>
          <div>
            <p className="text-xs text-[var(--color-text-muted)] mb-1">요금제</p>
            <p className="text-sm text-[var(--color-text)]">{order.plan}</p>
          </div>
          <div>
            <p className="text-xs text-[var(--color-text-muted)] mb-1">금액</p>
            <p className="text-sm font-[var(--font-mono)] text-[var(--color-text)]">${order.amount}</p>
          </div>
          <div>
            <p className="text-xs text-[var(--color-text-muted)] mb-1">상태</p>
            <Badge variant={statusBadgeVariant[order.status]}>
              {order.status === 'cancelled' ? '취소됨' : order.status === 'completed' ? '완료' : order.status === 'processing' ? '처리 중' : order.status === 'shipped' ? '배송 중' : '대기'}
            </Badge>
          </div>
        </div>
      </div>
...
export default function OrdersPage() {
  ...
  const columns: Column<Order>[] = [
    ...
    { key: 'customer', label: '고객', render: (o) => <span className="font-medium text-[var(--color-text)]">{o.customer}</span> },
    { key: 'plan', label: '요금제', render: (o) => <span className="text-[var(--color-text-secondary)]">{o.plan}</span> },
    { key: 'amount', label: '금액', render: (o) => <span className="font-[var(--font-mono)]">${o.amount}</span>, className: 'text-right' },
    { key: 'status', label: '상태', render: (o) => <Badge variant={statusBadgeVariant[o.status]}>{o.status === 'cancelled' ? '취소됨' : o.status === 'completed' ? '완료' : o.status === 'processing' ? '처리 중' : o.status === 'shipped' ? '배송 중' : '대기'}</Badge> },
    { key: 'date', label: '날짜', render: (o) => <span className="text-[var(--color-text-muted)]">{o.date}</span> },
  ]
    ...
    <DashboardLayout title="주문">
      ...
      >
        ...
        <div className="grid grid-cols-4 gap-3">
          {[
            { label: '전체', value: orderStats.total, color: 'var(--color-text)' },
            { label: '대기', value: orderStats.pending, color: 'var(--color-warning)' },
            { label: '배송 중', value: orderStats.shipped, color: 'var(--color-primary)' },
            { label: '완료', value: orderStats.completed, color: 'var(--color-success)' },
          ].map((stat) => (
        ...
        <div className="bg-[var(--color-bg-elevated)] rounded-[var(--radius-lg)] border border-[var(--color-border)] overflow-hidden">
          <div className="flex gap-1 p-3 border-b border-[var(--color-border)]">
            {statusFilter.map((s) => (
              ...
              >
                {s === 'all' ? '전체' : s === 'cancelled' ? '취소됨' : s === 'completed' ? '완료' : s === 'processing' ? '처리 중' : s === 'shipped' ? '배송 중' : '대기'}
              </button>
            ))}
          </div>
