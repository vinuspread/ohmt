  ...
  return (
    <div className="relative overflow-hidden rounded-[var(--radius-lg)] p-6 bg-gradient-to-br from-[var(--color-primary-muted)] to-[var(--color-bg-elevated)] border border-[var(--color-border)] h-full">
      <div className="relative z-10">
        <p className="text-sm text-[var(--color-text-muted)] mb-1">안녕하세요, Alex님</p>
        <h2 className="text-xl font-[var(--font-heading)] font-semibold text-[var(--color-text)]">
          VINUSPREAD Motors
        </h2>
        <p className="text-xs text-[var(--color-text-muted)] mt-1">운영 및 영업 대시보드</p>

        <div className="mt-6">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm text-[var(--color-text-secondary)]">Q2 납기 목표</span>
          ...
          <p className="text-xs text-[var(--color-text-muted)] mt-1">1,400대 중 1,204대 납기 완료</p>
        </div>
      </div>
    </div>
  )
