...
const navItems = [
  ...
  {
    name: "Shop",
    href: "/ko/templates/exhibition/souvenir",
    icon: ShoppingBag,
...
export default function MobileBottomTabBar({ isMenuOpen, onMenuToggle, onTicketClick }: MobileBottomTabBarProps) {
  ...
  const isActive = (href: string) => {
    if (href === "/ko/templates/exhibition" || href === "#") {
      return pathname === "/ko/templates/exhibition" || pathname === "/";
    }
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              ...
              href={item.href}
              onClick={handleClick}
               className={`flex flex-col items-center justify-center w-14 h-14 rounded-xl transition-[color,background] duration-300 ${
                 isActiveItem
