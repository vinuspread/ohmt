      ...
      <main>
        ...
        <section className="relative min-h-screen">
          <div className="absolute inset-0">
            <img src={exhibition.heroImage} alt={exhibition.name} className="w-full h-full object-cover" />
          ...
          <div className="absolute bottom-16 left-0 right-0 z-10 max-w-[1400px] mx-auto px-6">
            ...
            <h1 className="font-heading font-semibold uppercase text-white leading-[0.95]" style={{ fontSize: 'clamp(3rem, 6vw, 6rem)', letterSpacing: '-0.04em' }}>
              {exhibition.name}
            </h1>
          </div>
        </section>
        ...
        <section className="bg-[var(--color-bg)] py-24 md:py-32">
          <div className="max-w-[1400px] mx-auto px-6">
            <div className="grid md:grid-cols-2 gap-16">
              <div>
                <p className="text-[10px] font-body font-semibold uppercase tracking-[0.1em] text-black/40 mb-6">전시 소개</p>
                <p className="text-[18px] leading-[1.75] text-black/80" style={{ fontFamily: 'var(--font-body)' }}>{exhibition.description}</p>
              </div>
              <div>
                <div className="flex items-center gap-3 mb-10">
                  <span className="text-[10px] font-body font-semibold uppercase tracking-[0.1em] px-2 py-0.5 bg-black text-white">{exhibition.tags.ageRating}</span>
                  <span className="text-[10px] font-body font-semibold uppercase tracking-[0.1em] text-black/40">
                    {exhibition.status === 'on-show' ? '전시 중' : exhibition.status === 'opening-soon' ? '개막 예정' : '상설 전시'}
                  </span>
                </div>
                <div className="flex flex-col gap-8 border-t border-[var(--color-border)] pt-10">
                  {[
                    { label: '작가', value: exhibition.artist },
                    { label: '기간', value: `${exhibition.dateFrom} - ${exhibition.dateTo}` },
                    { label: '전시장', value: exhibition.showroom },
                  ].map((meta) => (
                ...
                <Link href="/ko/templates/OHMT003-exhibition-KO/contact" className="relative overflow-hidden group inline-flex px-8 py-4 border border-black mt-12">
        ...
        <section className="bg-[var(--color-bg)] pb-32">
          <div className="max-w-[1400px] mx-auto px-6">
            <div className="flex items-baseline justify-between mb-10 border-t border-[var(--color-border)] pt-12">
              <p className="text-[10px] font-body font-semibold uppercase tracking-[0.12em] text-black/40">전시 현장</p>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {galleryImages.map((src, i) => (
                <div key={src} className="overflow-hidden" style={{ aspectRatio: i % 3 === 1 ? '3/4' : '4/3' }}>
        ...
        <section className="bg-[var(--color-bg)] pb-32 border-t border-[var(--color-border)]">
          <div className="max-w-[1400px] mx-auto px-6 pt-16">
            <div className="flex items-baseline justify-between mb-12">
              ...
              <Link href="/ko/templates/OHMT003-exhibition-KO/exhibitions" className="text-[11px] font-body font-semibold uppercase tracking-[0.12em] text-black border-b border-black pb-0.5 hover:opacity-60 transition-opacity">전체 보기</Link>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
              {related.map((ex) => (
                <Link key={ex.slug} href={`/ko/templates/OHMT003-exhibition-KO/exhibitions/${ex.slug}`} className="group block">
                  ...
                  <p className="text-[10px] font-body font-semibold uppercase tracking-[0.08em] text-black/40 mb-2">{ex.tags.genre} · {ex.tags.theme}</p>
                  <p className="text-[14px] font-heading font-semibold uppercase tracking-[-0.01em] text-black">{ex.name}</p>
                </Link>
              ))}
            </div>
          </div>
