      ...
      <div className="mx-auto max-w-[1440px]">
        <div className="mb-6 grid gap-6 border-t border-[var(--color-text)] pt-6 md:mb-12 md:grid-cols-2 md:items-end md:pt-6">
          ...
          <p className="max-w-[480px] text-base leading-relaxed text-[var(--color-text-muted)] md:justify-self-end">
            참여자의 이야기는 사진과 글로 전하고, 프로그램의 성과는 확인 가능한 수치로 공개합니다.</p>
        </div>
        ...
        >
          {stats.map((stat) => (
            <motion.div
              ...
              className="py-6 md:border-r md:border-[#d8dee5] md:px-12 md:py-14 md:last:border-r-0"
            >
              <p className="font-heading text-7xl font-bold leading-none text-[var(--color-text)] md:text-7xl">
                {stat.prefix}
                <CountUp value={stat.value} decimals={stat.decimals} />
                <span className="font-bold text-[var(--color-primary)]">{stat.suffix}</span>
              </p>
              <p className="mt-6 text-base font-normal leading-[var(--leading-heading)] text-[var(--color-text-muted)]">
                {stat.label}
              </p>
            </motion.div>
          ))}
