        ...
        {SEASONS.map((season, i) => (
          <section key={season.id} className="max-w-[1440px] mx-auto px-6 md:px-12 py-24 md:py-36">
            <div className={`grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-20 items-center ${i % 2 === 1 ? "md:grid-flow-dense" : ""}`}>
              ...
              >
                <span className="text-xs font-semibold tracking-[-0.01em] text-white/40">
                  {season.label}
                </span>
                <h2 className="mt-3 mb-5 text-2xl font-light leading-[1.2] tracking-[-0.025em] text-white sm:text-3xl md:text-4xl break-keep">
                  {season.title}
                </h2>
                <p className="max-w-[52ch] text-base leading-[1.7] tracking-[-0.015em] text-white/60 break-keep">
                  {season.description}
                </p>
