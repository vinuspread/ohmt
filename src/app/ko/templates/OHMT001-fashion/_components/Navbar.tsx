    ...
    <>
      <nav className={`fixed top-0 left-0 right-0 z-50 h-14 md:h-20 transition-[background,border-color] duration-700 flex items-center ${!isTransparent ? "bg-white/95 backdrop-blur-xl border-b border-black/5" : "bg-transparent"}`}>
        <div className="max-w-[1440px] mx-auto w-full px-6 md:px-12 flex justify-between items-center">
          <Link href="/ko/templates/OHMT001-fashion" className={`text-xl md:text-2xl font-black tracking-tighter uppercase transition-colors duration-500 ${!isTransparent ? "text-black" : "text-white"}`}>
            OHMT
          </Link>
           ...
           <div className={`hidden md:flex items-center gap-10 text-[14px] font-bold uppercase tracking-[0.3em] transition-colors duration-500 ${!isTransparent ? "text-black/60" : "text-white/80"}`}>
             {[
                 { name: "컬렉션", id: "collection", href: "/ko/templates/OHMT001-fashion/category/collection" },
                 { name: "아카이브", id: "archive", href: "/ko/templates/OHMT001-fashion/category/archive" },
                 { name: "저널", id: "journal", href: "/ko/templates/OHMT001-fashion/journal" },
                 { name: "브랜드", id: "about", href: "/ko/templates/OHMT001-fashion/about" },
               ].map((item) => {
                 const isActive = item.id === "journal" || item.id === "about"
    ? pathname.includes(`/ko/templates/OHMT001-fashion/${item.id}`)
    : pathname.includes(`/ko/templates/OHMT001-fashion/category/${item.id}`);
               return (
                 <Link
                   key={item.id}
                   href={item.href}
           ...
           <div className={`flex items-center gap-4 sm:gap-6 transition-colors duration-500 ${!isTransparent ? "text-black" : "text-white"}`}>
             ...
             <Link href="/ko/templates/OHMT001-fashion/cart" className="relative hover:opacity-50 transition-opacity">
