    ...
    <footer className="bg-[var(--color-primary)] pt-16">
      <div className="max-w-[1400px] mx-auto px-6">
        <nav className="flex gap-10">
          {navLinks.map((link) => (
            <Link key={link.href} href={link.href} className="text-xs font-body font-semibold text-white/60 hover:text-white transition-colors duration-200 tracking-tight">
              {link.label}
            </Link>
          ))}
        </nav>
        ...
        <div className="flex justify-between items-center mt-8">
          <p className="text-sm text-white/40">&copy; 2026 VANTA GALLERY.</p>
