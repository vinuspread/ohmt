...
const categoryServiceMap: Record<string, string[]> = {
  "스킨케어": ["hydrafacial", "chemical-peels"],
  "트리트먼트": ["microneedling", "laser-resurfacing"],
  "웰니스": ["aromatherapy", "body-massage"],
};
...
export default function BlogFull() {
  ...
  const rest = blogPosts.filter((p) => p.id !== featured.id).filter((p) => activeCategory === "전체" || p.category === activeCategory);
    ...
    <>
      ...
      <main>
        <section className="relative min-h-[50dvh] overflow-hidden bg-[var(--color-secondary)]">
          <img src="/templates/OHMT026-spa/blog-detail-01.jpg" alt="" className="absolute inset-0 h-full w-full object-cover opacity-40" />
          ...
          <div className="relative z-10 mx-auto max-w-[1440px] px-6 flex h-full min-h-[50dvh] flex-col justify-end pb-16 lg:pb-20">
            <motion.div initial={reduce ? { opacity: 0 } : { opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: EASE_OUT }} className="max-w-2xl">
              ...
              <h1 className="mt-4 font-[family-name:var(--font-heading)] text-6xl font-bold tracking-tight text-[var(--color-text-contrast)] leading-[var(--leading-heading)]">스킨케어 노트</h1>
        ...
        <section className="py-16 bg-[var(--color-bg)]">
          <div className="mx-auto max-w-[1440px] px-6">
            <motion.a href={`/ko/templates/OHMT026-spa/blog/${featured.id}`} initial={reduce ? { opacity: 0 } : { opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: EASE_OUT }} className="group grid gap-8 lg:grid-cols-2 items-center rounded-sm overflow-hidden border border-[var(--color-border)] bg-[var(--color-bg-secondary)]">
                ...
                <motion.div key={post.id} initial={reduce ? { opacity: 0 } : { opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ duration: 0.5, delay: i * 0.08, ease: EASE_OUT }} className="group rounded-2xl overflow-hidden bg-[var(--color-bg-secondary)] border border-[var(--color-border)] hover:-translate-y-1 transition-all duration-300">
                  <a href={`/ko/templates/OHMT026-spa/blog/${post.id}`} className="block">
                    ...
                    <div className="p-6 pb-0">
                      ...
                      <h2 className="mt-3 font-[family-name:var(--font-heading)] text-lg font-bold tracking-tight text-[var(--color-text)] leading-[var(--leading-heading)] group-hover:text-[var(--color-primary)] transition-colors duration-200">{post.title}</h2>
                  ...
                  <div className="p-6 pt-4 flex items-center justify-between">
                    ...
                    {categoryServiceMap[post.category] && (
                      <a href={`/ko/templates/OHMT026-spa/service#${categoryServiceMap[post.category][0]}`} className="text-xs font-semibold text-[var(--color-primary)] hover:underline">관련 트리트먼트 →</a>
                    )}
                  </div>
                </motion.div>
              ))}
