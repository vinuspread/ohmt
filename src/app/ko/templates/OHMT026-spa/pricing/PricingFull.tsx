...
const inclusions = [
  { icon: Stethoscope, label: "무료 초기 상담" },
  { icon: PackageCheck, label: "배리어 테스트" },
  { icon: FileClock, label: "애프터케어 가이드" },
];
      ...
      <main>
        <section className="relative min-h-[50dvh] overflow-hidden bg-[var(--color-secondary)]">
          ...
          <div className="relative z-10 mx-auto max-w-[1440px] px-6 flex h-full min-h-[50dvh] flex-col justify-end items-center pb-16 lg:pb-20 text-center">
            <motion.div initial={reduce ? { opacity: 0 } : { opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: EASE_OUT }}>
              <h1 className="font-[family-name:var(--font-heading)] text-6xl font-bold tracking-tight text-[var(--color-text-contrast)] leading-[var(--leading-heading)]">가격 플랜</h1>
        ...
        <section className="py-24 bg-[var(--color-bg-secondary)]">
          <div className="mx-auto max-w-5xl px-6">
            <motion.div initial={reduce ? { opacity: 0 } : { opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ duration: 0.6, ease: EASE_OUT }} className="mb-12">
              <span className="text-[0.7rem] font-bold uppercase tracking-[0.2em] text-[var(--color-primary)]">비교</span>
              <h2 className="mt-4 font-[family-name:var(--font-heading)] text-3xl font-bold tracking-tight text-[var(--color-text)] leading-[var(--leading-heading)]">플랜 비교</h2>
            </motion.div>
              ...
              <table className="w-full text-sm border-collapse min-w-[560px]">
                <thead>
                  <tr className="border-b border-[var(--color-border)]">
                    ...
                    <th className="text-center font-[family-name:var(--font-heading)] font-bold text-[var(--color-text)] py-4 px-4">베이직</th>
                    <th className="text-center font-[family-name:var(--font-heading)] font-bold text-[var(--color-primary)] py-4 px-4">어드밴스드</th>
                    <th className="text-center font-[family-name:var(--font-heading)] font-bold text-[var(--color-text)] py-4 px-4">프리미엄</th>
                  </tr>
                </thead>
                <tbody>
        ...
        <section className="py-24 bg-[var(--color-bg)]">
          <div className="mx-auto max-w-3xl px-6">
            <motion.div initial={reduce ? { opacity: 0 } : { opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ duration: 0.6, ease: EASE_OUT }} className="mb-12">
              <span className="text-[0.7rem] font-bold uppercase tracking-[0.2em] text-[var(--color-primary)]">결제 안내</span>
              <h2 className="mt-4 font-[family-name:var(--font-heading)] text-3xl font-bold tracking-tight text-[var(--color-text)] leading-[var(--leading-heading)]">결제 & 취소 정책</h2>
            </motion.div>
