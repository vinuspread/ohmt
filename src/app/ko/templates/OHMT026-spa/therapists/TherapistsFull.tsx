...
const specializations = ["레이저 피부 관리", "페이셜 리프레시", "필링 케어", "마이크로니들 케어", "마사지 테라피", "아로마테라피", "바디 케어", "피부 상담"];
        ...
        <section className="bg-[var(--color-secondary)] py-24 lg:py-32">
          <div className="mx-auto max-w-7xl px-6">
            <motion.div initial={reduce ? { opacity: 0 } : { opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: EASE_OUT }} className="max-w-2xl">
              ...
              <h1 className="mt-4 font-[family-name:var(--font-heading)] text-6xl font-bold tracking-tight text-[var(--color-text-contrast)] leading-[var(--leading-heading)]">케어를 담당하는 전문가</h1>
              <p className="mt-4 max-w-2xl text-[0.95rem] leading-relaxed text-white/60">
                <span className="md:block">각 분야의 경험을 갖춘 전문가가 피부 상태와 원하는 관리 방향에 맞춰</span>{" "}
                <span className="md:block">상담과 케어를 진행합니다.</span>
              </p>
            </motion.div>
          </div>
        </section>
              ...
              {team.map((member, i) => (
                <motion.div key={member.id} id={member.id} initial={reduce ? { opacity: 0 } : { opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ duration: 0.5, delay: i * 0.1, ease: EASE_OUT }} className="group rounded-2xl overflow-hidden bg-[var(--color-bg-secondary)] border border-[var(--color-border)] scroll-mt-28">
                  ...
                  <div className="p-6">
                    ...
                    <a href="/ko/templates/OHMT026-spa/contact" className="mt-4 inline-flex items-center text-sm font-semibold text-[var(--color-primary)] hover:underline">{member.name.split(" ")[0]} 상담 예약</a>
                  </div>
                </motion.div>
              ))}
            </div>
