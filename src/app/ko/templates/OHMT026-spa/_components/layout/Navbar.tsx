  ...
  return (
    <header
      className={`fixed top-0 inset-x-0 z-50 transition-all duration-300 ${
        useSolidHeader ? "bg-[var(--color-bg)]/95 backdrop-blur-sm shadow-sm" : "bg-transparent"
      }`}
    >
      <nav className="mx-auto max-w-7xl px-6 py-4 flex items-center justify-between">
        ...
        >
          <span className="w-6 h-px" style={{ backgroundColor: useSolidHeader ? "var(--color-text)" : "var(--color-text-contrast)" }} />
          <span className="w-6 h-px" style={{ backgroundColor: useSolidHeader ? "var(--color-text)" : "var(--color-text-contrast)" }} />
        </button>
      </nav>
