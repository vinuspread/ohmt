      ...
      <div className="mx-auto max-w-[1440px] px-6">
        <div className="grid items-center gap-14 lg:grid-cols-2 lg:gap-24">
          <motion.div initial={reduce ? { opacity: 0 } : { opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ duration: 0.6, ease: EASE_OUT }} className="max-w-2xl">
            ...
            <h2 className="mt-4 max-w-xl font-[family-name:var(--font-heading)] text-5xl font-bold tracking-tight text-[var(--color-text-contrast)] leading-[var(--leading-heading)]">당신의 피부는 전문가의 손에</h2>
          ...
          <motion.div initial={reduce ? { opacity: 0 } : { opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ duration: 0.6, delay: 0.15, ease: EASE_OUT }} className="relative overflow-hidden rounded-sm">
            <img src="/templates/OHMT026-spa/who-we-are.jpg" alt="상담실에서 고객을 맞이하는 테라피스트 팀" className="w-full h-[26rem] object-cover" />
            <div className="absolute inset-x-0 bottom-0 grid grid-cols-3 bg-black/40 backdrop-blur-sm border-t border-white/10">
              {stats.map((stat) => (
                <div key={stat.label} className="border-l border-white/10 first:border-l-0 px-4 py-5 text-left">
                  <div className="font-[family-name:var(--font-heading)] text-2xl font-bold text-[var(--color-primary)] leading-none"><Counter value={stat.value} suffix={stat.suffix} /></div>
