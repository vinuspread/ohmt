    ...
    <section className="relative overflow-hidden bg-[var(--color-bg)] py-24 lg:py-32">
      <div className="mx-auto max-w-7xl px-6">
        <motion.div initial={reduce ? { opacity: 0 } : { opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ duration: 0.6, ease: EASE_OUT }} className="text-center mb-16">
          <span className="text-[0.7rem] font-bold uppercase tracking-[0.2em] text-[var(--color-primary)]">멤버십</span>
          <h2 className="mt-4 font-[family-name:var(--font-heading)] text-5xl font-bold tracking-tight text-[var(--color-text)] leading-[var(--leading-heading)]">필요에 따라 선택하는 멤버십</h2>
          <p className="mt-5 text-[0.95rem] text-[var(--color-text-muted)] leading-relaxed max-w-md mx-auto">이용 횟수와 필요한 관리 범위에 맞는 멤버십을 선택하세요.</p>
        </motion.div>
          ...
          {pricingPlans.map((plan, i) => (
            <motion.div key={plan.id} initial={reduce ? { opacity: 0 } : { opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ duration: 0.5, delay: i * 0.1, ease: EASE_OUT }} className={`flex h-full flex-col rounded-2xl border p-8 lg:p-10 ${plan.featured ? "border-[var(--color-primary)] bg-[var(--color-primary)]/5 relative" : "border-[var(--color-border)] bg-[var(--color-bg-secondary)]"}`}>
              ...
              <ul className="mb-8 mt-8 space-y-3">
                {plan.features.map((f) => (
                  <li key={f} className="flex items-start gap-3 text-sm text-[var(--color-text-muted)]"><span className="mt-0.5 w-1.5 h-1.5 rounded-full bg-[var(--color-primary)] shrink-0" />{f}</li>
                ))}
              </ul>
