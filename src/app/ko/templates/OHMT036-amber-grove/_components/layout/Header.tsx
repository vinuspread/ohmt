  ...
  return (
    <header className="sticky top-0 z-50 bg-white -mx-4 sm:-mx-6 lg:mx-0 px-4 sm:px-6 lg:px-0">
      <nav className="flex h-16 items-center justify-between">
        <Link href={base} className="font-[family-name:var(--font-body)] text-lg font-bold tracking-[0.08em] text-[var(--color-bg-dark)]">
          OHMT
        </Link>
          ...
          >
            <div className="flex h-full flex-col">
              ...
              <div className="flex flex-1 flex-col justify-center px-8 pb-24">
                <nav className="flex flex-col gap-6">
                  {navItems.map((item) => (
                    <Link
                      ...
                      className="group"
                    >
                      <p className="font-[family-name:var(--font-heading)] text-3xl font-semibold leading-[var(--leading-body)] text-[var(--color-text)] transition-colors duration-200 group-hover:text-[var(--color-accent)] sm:text-4xl">
                        {item.label}
                      </p>
                ...
                <Link
                  href={`${base}/shop/orchard-gift-crate`}
                  onClick={() => setOpen(false)}
