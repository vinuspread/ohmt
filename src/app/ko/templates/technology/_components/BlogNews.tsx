  ...
  return (
    <section id="news" className="bg-[var(--color-bg-secondary)] py-20 md:py-32 border-b border-[var(--color-border)]">
      <div className="mx-auto max-w-7xl px-6">
        ...
        {/* Layout - fixed height so both columns match exactly */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:h-[406px]">
          {/* Left: Featured image with text overlay */}
          {featuredPost && (
            <Link href="#news" className="lg:col-span-7 group relative block h-full overflow-hidden rounded-2xl">
              <img
          ...
          <div className="lg:col-span-5 flex flex-col justify-between h-full divide-y divide-[var(--color-border)]/50">
            {listPosts.map((post) => (
              <Link
                key={post.id}
                href="#news"
