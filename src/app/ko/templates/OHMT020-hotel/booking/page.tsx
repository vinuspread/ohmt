        ...
        <section className="relative h-[40vh] md:h-[50vh] flex items-center bg-[var(--color-primary)] overflow-hidden">
          ...
          <div className="relative z-10 max-w-[1440px] mx-auto px-6 md:px-12 w-full pt-20">
            <motion.div initial={{ opacity: 0, y: 24, filter: "blur(6px)" }} animate={{ opacity: 1, y: 0, filter: "blur(0px)" }} transition={{ duration: 0.75, ease: EASE }}>
              <span className="text-xs font-medium text-[var(--color-accent)] uppercase tracking-[0.25em] block mb-3">투숙을 계획하세요</span>
          ...
          <div className="max-w-[1440px] mx-auto px-6 md:px-12">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20 items-start">
              <div className="lg:col-span-4 flex flex-col gap-8">
                <div>
                  <span className="text-xs font-semibold text-[var(--color-accent)] uppercase tracking-[0.2em] block mb-3">객실 타입 선택</span>
              ...
              <div className="lg:col-span-8 bg-white border border-[var(--color-border)] p-8 md:p-12">
                {submitted ? (
                  <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.4, ease: EASE }} className="text-center py-16 flex flex-col items-center gap-6">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-[var(--color-text-muted)] max-w-md mx-auto">
                        CASSIA를 선택해 주셔서 감사합니다. 예약 요청이 접수되었으며, 담당 팀이 24시간 이내에 세부 내용을 확인해 드립니다.
                      </p>
                    </div>
                    <Button variant="primary" size="md" onClick={() => setSubmitted(false)} className="text-xs tracking-[0.2em] uppercase rounded-sm mt-2">다시 예약하기</Button>
                  </motion.div>
                ) : (
                  <form onSubmit={handleSubmit} className="flex flex-col gap-6">
                    ...
                    <div>
                      <label className="text-xs font-semibold text-[var(--color-text-muted)] uppercase tracking-wider block mb-2">요청 사항</label>
                    ...
                    <div>
                      ...
                      <p className="text-sm text-[var(--color-text-muted)] text-center mt-3">이 단계에서는 결제가 진행되지 않습니다. 담당 팀이 확인 후 연락드립니다.</p>
                    </div>
                  </form>
                )}
              </div>
