        ...
        <section className="relative h-[40vh] md:h-[50vh] flex items-center bg-[var(--color-primary)] overflow-hidden">
          ...
          <div className="relative z-10 max-w-[1440px] mx-auto px-6 md:px-12 w-full pt-20">
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
              <span className="text-xs font-medium text-[var(--color-accent)] tracking-tight block mb-3">고객 지원</span>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20 items-start">
              ...
              <div className="lg:col-span-5 flex flex-col gap-10">
                ...
                <div className="aspect-[16/10] bg-[var(--color-bg-secondary)] border border-[var(--color-border)] relative flex flex-col items-center justify-center p-8 text-center text-xs text-[var(--color-text-muted)] overflow-hidden">
                  ...
                  <span className="relative z-10 max-w-xs leading-relaxed">지도를 불러오는 중입니다. 리조트는 코사무이 국제공항(USM)에서 전용 셔틀로 약 15분 거리에 있습니다.</span>
                </div>
              </div>
              ...
              <div className="lg:col-span-7 bg-white border border-[var(--color-border)] p-8 md:p-12">
                {submitted ? (
                  <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center py-16 flex flex-col items-center gap-6">
                    ...
                    <Button variant="primary" size="md" onClick={() => setSubmitted(false)} className="text-xs tracking-tight mt-4">새로운 메시지 보내기</Button>
                  </motion.div>
                ) : (
                  <form onSubmit={handleSend} className="flex flex-col gap-6">
                    <div>
                      <h3 className="text-2xl font-[var(--font-heading)] font-bold text-[var(--color-primary)] mb-2">리조트 문의 작성</h3>
                      <p className="text-sm text-[var(--color-text-muted)]">맞춤 패키지 조율이나 일반 문의가 필요하시면 아래 양식으로 보내주세요.</p>
                    </div>
                    ...
                    <div className="flex flex-col gap-4">
                      ...
                      <div>
                        <label className="text-xs font-semibold text-[var(--color-text-muted)] tracking-tight block mb-2">상세 문의 내용 *</label>
