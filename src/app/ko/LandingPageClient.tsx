...
const serviceCardsKo = [
  { icon: Wrench, title: "직접 만들면 금방 흐트러진다", desc: "툴은 쉬워도 구조, 이미지, 카피를 맞추는 순간 완성도가 쉽게 무너집니다." },
  { icon: ImageOff, title: "콘텐츠 준비에서 멈춘다", desc: "멋진 레이아웃도 들어갈 문장과 사진이 없으면 실제 사이트가 되지 않습니다." },
  { icon: Shuffle, title: "어떤 흐름이 맞는지 어렵다", desc: "첫 화면, 섹션 순서, 문의 흐름을 정하지 못하면 제작 내내 기준이 흔들립니다." },
  { icon: ClipboardList, title: "외주는 시작부터 무겁다", desc: "견적, 일정, 피드백 비용이 커서 작은 사이트도 프로젝트가 크게 번집니다." },
  { icon: PackageOpen, title: "오픈 후 운영이 남는다", desc: "구매나 제작으로 끝나지 않습니다. 수정, 업데이트, 유지보수까지 이어져야 합니다." },
];
            ...
            {packages.map((pkg) => (
              ...
              >
                ...
                <button
                  ...
                  className={`w-full mt-8 py-3 text-xs font-bold transition-all duration-150 rounded-md ${
