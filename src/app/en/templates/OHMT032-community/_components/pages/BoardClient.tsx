    ...
    <div className="space-y-5">
      <section className="rounded-[var(--radius-lg)] bg-[var(--color-bg-secondary)] p-6">
        ...
        <h1 className="mt-2.5 text-3xl font-semibold leading-[var(--leading-heading)] text-[var(--color-text)] md:text-5xl">
          {invalidCategory ? 'Board not found' : category?.description ?? 'Browse every post in the community'}
        </h1>
      </section>
      ...
      {visiblePosts.length > 0 ? (
        ...
        <div className="rounded-[var(--radius-lg)] border border-[var(--color-border)] bg-[var(--color-bg-elevated)] p-6 text-center">
          <h2 className="text-xl font-semibold text-[var(--color-text)]">No posts found</h2>
          <p className="mt-2 text-sm text-[var(--color-text-muted)]">Change your filters or return to the full board.</p>
      ...
      {filtered.length > pageSize && (
        <div className="flex flex-wrap items-center justify-center gap-2">
          <button type="button" onClick={() => setPage((value) => Math.max(1, value - 1))} className="rounded-full p-1.5 hover:bg-[var(--color-bg-hover)]" aria-label="Previous page">
          ...
          <button type="button" onClick={() => setPage((value) => Math.min(totalPages, value + 1))} className="rounded-full p-1.5 hover:bg-[var(--color-bg-hover)]" aria-label="Next page">
