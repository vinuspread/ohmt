  ...
  return (
    <article className="rounded-[var(--radius-lg)] border border-[var(--color-border)] bg-[var(--color-bg-elevated)] p-6 transition hover:border-[var(--color-border-strong)] hover:bg-white">
      <div className="mb-3 flex flex-wrap items-center gap-1.5">
        ...
        <Link
          href={`/en/templates/OHMT032-community/board/${post.category}`}
      ...
      <Link href={`/en/templates/OHMT032-community/post/${post.id}`} className="group block">
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2.5 text-xs text-[var(--color-text-muted)]">
          ...
          <span className="inline-flex items-center gap-1"><Timer size={13} strokeWidth={1.8} className="text-[var(--color-accent)]" />{post.readMinutes} min</span>
        </div>
      </div>
    </article>
  )
