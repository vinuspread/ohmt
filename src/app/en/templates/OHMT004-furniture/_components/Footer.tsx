        ...
        {items.map((item, i) => (
          <div key={i} className="flex items-center gap-6 group">
            ...
            <div>
               ...
               <p className="text-sm font-medium text-[var(--color-secondary)]">{item.desc}</p>
            </div>
          </div>
        ))}
      </div>
    ...
    <footer className="bg-white text-[var(--color-primary)] py-12 md:py-20 border-t border-black/5">
      <div className="max-w-[1440px] mx-auto px-6 md:px-12 flex flex-col md:flex-row justify-between items-center gap-8 md:gap-12">
        <div className="text-center md:text-left">
          <h2 className="text-xl md:text-2xl font-medium md:font-extrabold lowercase mb-1">KRAFT</h2>
          <p className="text-xs text-[var(--color-secondary)] font-medium uppercase">Essential Interior Archive</p>
        </div>

        <div className="flex items-center gap-8 text-xs font-medium text-[var(--color-secondary)] uppercase">
        ...
        <div className="flex flex-col items-center md:items-end gap-3 text-xs font-medium uppercase text-black/30">
