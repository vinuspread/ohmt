...
const ProductCard = ({ product, index }: { product: Product; index: number }) => {
  return (
    ...
    >
      <Link href={`/en/templates/OHMT004-furniture/product/${product.id}`}>
      ...
      >
        <Link href={`/en/templates/OHMT004-furniture/product/${product.id}`} className="min-w-0">
          <h3 className="text-base font-bold text-[var(--color-text)] group-hover:text-black transition-colors uppercase">{product.name}</h3>
        </Link>
        <p className={`text-xs text-zinc-500 font-medium leading-[var(--leading-body)] w-full ${theme.typography.body.style}`}>
          {product.desc}
        </p>
    ...
    <section className="bg-white py-12 md:py-24 lg:py-36">
      <div className="max-w-[1440px] mx-auto px-6 md:px-12">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end mb-10 md:mb-16 lg:mb-24 gap-4 md:gap-12 border-b border-black/5 pb-6 md:pb-12">
          ...
          <button className="text-xs md:text-sm font-bold text-[var(--color-text)] border-b-2 border-black pb-1 hover:text-[var(--color-primary)] hover:border-[var(--color-primary)] transition-all whitespace-nowrap self-start sm:self-auto">
            {t.grid.cta}
          </button>
        </div>
