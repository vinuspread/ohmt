...
export function SectionHeading({
  ...
  return (
    <h2
      className={`${headingSize[size]} font-semibold leading-[var(--leading-heading)] tracking-[-0.02em] text-white ${className}`}
    >
      {children}
    </h2>
  );
...
export function BodyText({ children, className = "" }: TextProps) {
  return (
    <p className={`resort-body text-base font-normal text-white/70 ${className}`}>
      {children}
    </p>
  );
}
