    ...
    <>
      <nav className={`fixed top-0 left-0 right-0 z-50 h-14 md:h-20 transition-all duration-700 flex items-center ${!isTransparent ? "bg-white/95 backdrop-blur-xl border-b border-black/5 shadow-sm" : "bg-transparent"}`}>
        <div className="max-w-[1440px] mx-auto w-full px-6 md:px-12 flex justify-between items-center transition-all">
          <Link href="/en/templates/OHMT001-fashion-en-en" className={`text-xl md:text-2xl font-black tracking-tighter uppercase transition-colors duration-500 ${!isTransparent ? "text-black" : "text-white"}`}>
            Oh My Template
          </Link>
           ...
           <div className={`hidden md:flex items-center gap-10 text-[14px] font-bold uppercase tracking-[0.3em] transition-colors duration-500 ${!isTransparent ? "text-black/60" : "text-white/80"}`}>
             {[
                { name: "Collection", id: "collection", href: "/en/templates/OHMT001-fashion-en-en/category/collection" },
                { name: "Archive", id: "archive", href: "/en/templates/OHMT001-fashion-en-en/category/archive" },
                { name: "Journal", id: "journal", href: "/en/templates/OHMT001-fashion-en-en/category/journal" },
                { name: "About", id: "about", href: "/en/templates/OHMT001-fashion-en-en/category/about" },
              ].map((item) => {
                const isActive = pathname.includes(`/en/templates/OHMT001-fashion-en-en/category/${item.id}`);
               return (
                 <Link
                   key={item.id}
                   href={item.href}
           ...
           <div className={`flex items-center gap-4 sm:gap-6 transition-colors duration-500 ${!isTransparent ? "text-black" : "text-white"}`}>
             ...
             <Link href="/en/templates/OHMT001-fashion-en-en/cart" className="relative hover:opacity-50 transition-opacity">
        ...
        <div className="flex flex-col px-8 text-[13px] font-bold uppercase tracking-[0.25em] text-black/60">
          ...
          {/* Utilities */}
           <div className="flex flex-col gap-3">
             ...
             <Link
               href="/en/templates/OHMT001-fashion-en-en/cart"
               onClick={() => setMobileOpen(false)}
               className="hover:text-black py-2.5 transition-colors block"
             >
               Shopping Bag
              ...
              <Link
                href="/en/templates/OHMT001-fashion-en-en"
                onClick={() => setMobileOpen(false)}
