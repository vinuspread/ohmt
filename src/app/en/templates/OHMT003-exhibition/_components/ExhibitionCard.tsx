...
export default function ExhibitionCard({ exhibition }: { exhibition: Exhibition }) {
  return (
    <Link href={`/en/templates/OHMT003-exhibition/exhibitions/${exhibition.slug}`} className="group block cursor-pointer">
      ...
      <p
        className="mt-4 text-sm font-semibold uppercase tracking-[0.06em] text-[var(--color-text)]"
        style={{ fontFamily: 'var(--font-body)' }}
      >
        {exhibition.name}
      </p>
      <p
        className="mt-2 text-xs uppercase tracking-[0.06em] text-[var(--color-text-muted)]"
        style={{ fontFamily: 'var(--font-body)' }}
      >
