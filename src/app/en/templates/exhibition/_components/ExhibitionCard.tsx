...
export default function ExhibitionCard({ exhibition }: { exhibition: Exhibition }) {
  return (
    <Link href={`/en/templates/OHMT005-exhibition-en/exhibitions/${exhibition.slug}`} className="group block cursor-pointer">
      ...
      <p
        className="mt-4 text-[14px] font-semibold uppercase tracking-[0.06em] text-[var(--color-text)]"
        style={{ fontFamily: 'var(--font-body)' }}
      >
        {exhibition.name}
      </p>
      <p
        className="mt-2 text-[11px] uppercase tracking-[0.06em] text-[var(--color-text-muted)]"
        style={{ fontFamily: 'var(--font-body)' }}
      >
