...
export const Navbar = () => {
  ...
  const navItems = [
    { name: "Rooms", href: "/en/templates/OHMT020-hotel/rooms" },
    { name: "Services", href: "/en/templates/OHMT020-hotel/services" },
    { name: "Reservation", href: "/en/templates/OHMT020-hotel/booking" },
    { name: "Location", href: "/en/templates/OHMT020-hotel/contact" }
  ];
    ...
    <>
      <nav className={`fixed top-0 left-0 w-full h-16 md:h-20 z-50 transition-all duration-500 flex items-center ${scrolled || mobileOpen ? "bg-[var(--color-bg)]/95 backdrop-blur-xl border-b border-[var(--color-border)]" : "bg-transparent"}`}>
        <div className="max-w-[1440px] mx-auto w-full px-6 md:px-12 flex items-center justify-between">
          <Link href="/en/templates/OHMT020-hotel" className={`text-2xl md:text-3xl font-[var(--font-heading)] font-bold tracking-tight transition-colors duration-300 ${scrolled || mobileOpen ? "text-[var(--color-primary)]" : "text-white"}`}>
            OHMT
          </Link>
