...
export const Navbar = () => {
  ...
  const navItems = [
    { name: "Rooms", href: "/en/templates/hotel/rooms" },
    { name: "Services", href: "/en/templates/hotel/services" },
    { name: "Reservation", href: "/en/templates/hotel/booking" },
    { name: "Location", href: "/en/templates/hotel/contact" }
  ];
    ...
    <>
      <nav className={`fixed top-0 left-0 w-full h-16 md:h-20 z-50 transition-all duration-500 flex items-center ${scrolled || mobileOpen ? "bg-[var(--color-bg)]/95 backdrop-blur-xl border-b border-[var(--color-border)]" : "bg-transparent"}`}>
        <div className="max-w-[1440px] mx-auto w-full px-6 md:px-12 flex items-center justify-between">
          <Link href="/en/templates/hotel" className={`text-2xl md:text-3xl font-[var(--font-heading)] font-bold tracking-tight transition-colors duration-300 ${scrolled || mobileOpen ? "text-[var(--color-primary)]" : "text-white"}`}>
            OHMT
          </Link>
          ...
          <div className="flex items-center gap-4">
            <Link href="/en/templates/hotel/booking" className="hidden md:inline-flex">
      ...
      <div className={`fixed inset-x-0 top-16 z-40 bg-[var(--color-bg)] border-b border-[var(--color-border)] transition-all duration-300 md:hidden overflow-hidden ${mobileOpen ? "max-h-[400px] opacity-100 py-8" : "max-h-0 opacity-0 py-0"}`}>
        <div className="flex flex-col px-6 gap-2">
          ...
          <div className="mt-4 pt-4 border-t border-[var(--color-border)]">
            <Link href="/en/templates/hotel/booking" onClick={() => setMobileOpen(false)} className="w-full block">
