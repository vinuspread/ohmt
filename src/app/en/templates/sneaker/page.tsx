...
const bestSellers = products.filter(p => p.badge === "Best Seller" || p.rating >= 4.5).slice(0, 8);
  const newArrivals = products.filter(p => p.badge === "New" || !p.originalPrice).slice(0, 8);
      ...
      <div className="bg-white text-black selection:bg-black selection:text-white font-sans">
        <Suspense fallback={null}>
          ...
          <main>
            ...
            <FeatureRow />
            <CategoryBanners />
            <ProductGrid title={t.sectionTitles.bestSellers} items={bestSellers} limit={8} />
            <PromoBanner />
            <ProductGrid title={t.sectionTitles.newArrivals} items={newArrivals} limit={8} />
            <Reviews />
            <BlogSection />
          </main>
          <Footer />
