...
const models: Record<string, {
  ...
  ev9: {
    ...
    highlights: [
      ...
      { label: "0??00 km/h", value: "5.3 s" },
      { label: "Charging (10??0%)", value: "22 min" },
      ...
      <TemplateWrapper theme={theme}>
        <main className="antialiased bg-black text-white min-h-screen flex flex-col">
          ...
          <div className="flex-1 flex items-center justify-center flex-col gap-6 pt-12 md:pt-24">
            ...
            <Link href="/en/templates/OHMT009-car-EN/models" className="text-[0.75rem] uppercase tracking-[0.14em] text-[var(--theme-accent)] border-b border-[var(--theme-accent)] pb-0.5">??All Models</Link>
          </div>
        </main>
      </TemplateWrapper>
    );
      ...
      <main className="antialiased bg-black text-white selection:bg-[var(--theme-accent)] selection:text-black">
        ...
        <section className="relative h-[90vh] min-h-[560px] overflow-hidden flex items-end">
          ...
          <div className="relative w-full max-w-[var(--theme-container)] mx-auto px-6 md:px-[var(--theme-gutter)] pb-16 md:pb-24">
            ...
            <h1 className="font-[family-name:var(--theme-font-heading)] text-[clamp(4rem,10vw,9rem)] tracking-tight leading-[1.1] mb-4 text-white">
              {model.name}
            </h1>
            <p className="text-[0.95rem] text-white/60 font-normal max-w-[480px] mb-8">{model.tagline}</p>
            <div className="flex flex-col sm:flex-row gap-3">
              <Link href="/en/templates/OHMT009-car-EN/configure" className="inline-block text-[0.68rem] font-bold uppercase tracking-[0.16em] px-8 py-3 bg-[var(--theme-accent)] text-black hover:opacity-85 transition-opacity w-fit">
                Configure Your {model.name}
              </Link>
              <Link href="/en/templates/OHMT009-car-EN/models" className="inline-block text-[0.68rem] font-bold uppercase tracking-[0.16em] px-8 py-3 border border-white/25 text-white hover:border-white transition-colors w-fit">
                ??All Models
              </Link>
            </div>
          </div>
            ...
            <span className="text-[0.62rem] font-bold uppercase tracking-[0.3em] text-[var(--theme-text-muted)]">{model.name} Technology</span>
             {model.features.map((f) => (
               <div key={f.title} className="flex gap-6">
                 ...
                 <div>
                   <h3 className="text-[0.95rem] font-bold text-white mb-1.5 tracking-tight">{f.title}</h3>
                   <p className="text-[0.82rem] text-[var(--theme-text-muted)] leading-[1.4] font-normal">{f.desc}</p>
                 </div>
               </div>
             ))}
          </div>
        ...
        <section className="bg-[var(--color-primary)] border-t border-[var(--theme-border)]">
          <div className="max-w-[var(--theme-container)] mx-auto px-6 md:px-[var(--theme-gutter)] py-14 md:py-20 flex flex-col md:flex-row items-start md:items-center justify-between gap-8">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <Link href="/en/templates/OHMT009-car-EN/configure" className="text-[0.68rem] font-bold uppercase tracking-[0.16em] px-8 py-3.5 bg-[var(--theme-accent)] text-black hover:opacity-85 transition-opacity">
                Build & Price
              </Link>
              <Link href="/en/templates/OHMT009-car-EN" className="text-[0.68rem] font-bold uppercase tracking-[0.16em] px-8 py-3.5 border border-white/25 text-white hover:border-white transition-colors">
                Book Test Drive
              </Link>
            </div>
          </div>
