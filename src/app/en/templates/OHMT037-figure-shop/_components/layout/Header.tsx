    ...
    >
      ...
      {menuOpen ? (
        <div className="border-b border-[var(--color-border)] bg-[var(--color-bg)] px-4 pb-6 pt-2 lg:hidden">
          <nav aria-label="Mobile" className="flex flex-col">
            {NAV.map((item) => (
              <Link
                key={item.label}
                href={item.href}
                className="border-b border-[var(--color-border)] py-3 text-2xl font-medium text-[var(--color-ink)]"
          ...
          <p className="meta-label mt-6 text-[var(--color-ink-faint)]">Lines</p>
