  ...
  return (
    <div className="group bg-[var(--color-code-bg)] border border-[var(--color-border)] rounded-lg overflow-hidden">
      <div className="flex items-center justify-between px-4 py-1.5 border-b border-[var(--color-border)]">
        ...
        <button
          onClick={handleCopy}
          className="flex items-center gap-1 text-xs text-[var(--color-text-muted)] hover:text-[var(--color-accent)] transition-colors opacity-0 group-hover:opacity-100"
        >
          {copied ? (
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <main className="flex-1 overflow-y-auto flex items-center justify-center px-6">
            <div className="text-center space-y-5 max-w-sm">
              ...
              <div>
                <h1 className="text-xl font-bold text-[var(--color-text)] mb-1">Page not found</h1>
                <p className="text-sm text-[var(--color-text-muted)] leading-relaxed">
                  This page doesn&apos;t exist or has been moved. Try searching or browse from the sidebar.
                </p>
              </div>
              <Link
                href="/en/templates/docsEN"
              ...
              <div className="mt-12 pt-8 border-t border-[var(--color-border)]">
                ...
                <div className="flex flex-wrap gap-2">
                  {siblings.map((sibling) => (
                    <Link
                      key={sibling.slug}
                      href={`/en/templates/docsEN/${sibling.slug}`}
