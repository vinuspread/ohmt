          ...
          <div>
            ...
            <nav className="flex flex-col gap-4">
              {["Stock Information", "Financial Results", "SEC Filings", "Shareholder Services"].map(item => (
                <Link key={item} href="#" className="text-[0.8rem] text-[var(--color-text-muted)] hover:text-[var(--color-dark-bg)] transition-colors">{item}</Link>
              ))}
            </nav>
          </div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-center pt-10 border-t border-[var(--color-border)] gap-6">
          ...
          <div className="flex gap-8">
            {["Privacy Policy", "Terms of Use", "Stock Disclaimer"].map(item => (
              <Link key={item} href="#" className="text-[0.7rem] text-[var(--color-text-muted)] hover:text-[var(--color-dark-bg)] uppercase tracking-widest transition-colors">{item}</Link>
            ))}
          </div>
        </div>
      </div>
