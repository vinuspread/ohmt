...
export const Header = ({ light, t }: { light?: boolean; t: any }) => {
  ...
  return (
    <>
      <nav className={clsx(
        "fixed top-0 left-0 right-0 z-[200] h-[52px] flex items-center transition-all duration-300",
        scrolled ? "bg-[var(--color-bg)]/96 backdrop-blur-md" : "bg-transparent"
      )}>
        <div className="max-w-[var(--theme-container)] mx-auto px-6 md:px-[var(--theme-gutter)] flex items-center justify-between w-full">
          ...
          <div className="hidden md:flex gap-8 items-center">
            ...
            <button className={clsx(
              "text-[0.72rem] font-bold uppercase tracking-[0.1em] px-[1.2rem] py-[0.5rem] border transition-all duration-300",
              scrolled
      ...
      <div className={clsx(
        ...
        <div className="flex flex-col px-6 py-6 gap-1">
          ...
          <button className="mt-5 text-[0.85rem] font-bold uppercase tracking-[0.1em] px-6 py-3 border border-[var(--theme-text)] text-[var(--theme-text)] hover:bg-[var(--theme-text)] hover:text-white transition-all duration-300 w-fit">
            {t.nav.subscribe}
          </button>
        </div>
      </div>
